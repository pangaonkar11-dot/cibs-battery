...
export default function App() {
  ...
  const [ci, setCi] = useState({name:"",fileNo:"",dob:"",age:"",gender:"",school:"",grade:"",examiner:"",date:new Date().toISOString().slice(0,10),setting:"",purpose:""});
  ...
  if (step===6) {
    ...
    return wrap(<>
      ...
      <div style={{display:"flex",gap:10,flexWrap:"wrap",marginBottom:14}}>
        ...
        <button onClick={()=>{ makeVCSV(ci,cogScores,used,dx,sev,risk,imp); const ok=submitToDatabank_V(ci,cogScores,used,dx,sev,risk,imp,ci.examiner); setDbSubmitted_V(ok); }} style={{padding:"10px 20px",borderRadius:8,background:"#16a34a",color:"#fff",border:"none",fontSize:13,fontWeight:700,cursor:"pointer"}}>📊 Download CSV (CIBS Databank format)</button>
        ...
        {dbSubmitted_V&&<div style={{padding:"10px 14px",borderRadius:8,background:"#f0fdf4",border:"1px solid #86efac",fontSize:12,fontWeight:700,color:"#15803d"}}>✅ V Assessment submitted to CIBS Databank (File: {ci.fileNo})</div>}
      </div>
