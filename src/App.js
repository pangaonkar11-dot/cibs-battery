...
export default function CIBSBattery() {
  ...
  const vistaResultsRef = useRef(null); // Immediate access without async state delay
  ...
  const runVistaReport = async (sSeq, cSeq, shSeq, smSeq, demo) => {
    ...
    const vr = { clinical:cl, narratives:narr, reportId:vistaReportId.current };
    vistaResultsRef.current = vr;
    setVistaResults(vr);
  };
  ...
  const handleDemographics = async (demo) => {
    ...
    try {
      ...
      // Score VISTA — use ref if available (already computed), else compute fresh
      let vistaScored = vistaResultsRef.current?.clinical || null;
      if (!vistaScored && shapeSeq.length > 0) {
        try { vistaScored = computeClinical(shapeSeq, colorSeq, shadeSeq,
          smileySeq.length > 0 ? smileySeq : shapeSeq); } catch(e) { console.error("VISTA score error:",e); }
      }

      // Score VALID — read directly from state snapshot captured in closure
      const currentValidResp = validResp;
      let vBFI=null, vDuke=null, vCSS=null, vAUD=null, vCAT=null;
      if (currentValidResp) {
        try { vBFI  = scoreBFI(currentValidResp.d2); } catch(e) {}
        try { vDuke = scoreDuke(currentValidResp.d3); } catch(e) {}
        try { vCSS  = scoreCSS(Object.fromEntries(CSSRS.map((_,i)=>[`css${i+1}`, currentValidResp.d4?.[`css${i+1}`]]))); } catch(e) {}
        try { vAUD  = scoreAUDIT(Object.fromEntries(AUDITC.map((_,i)=>[`${i}`, currentValidResp.d4?.[`aud${i+1}`]??0]))); } catch(e) {}
        try { vCAT  = scoreCAT(currentValidResp.d1); } catch(e) {}
      }

      const r = vistaScored || {};
      console.log("📊 Scoring — VISTA CQ:", r?.d1?.CQ, "| VALID CQ:", vCAT?.iq, "| BFI-O:", vBFI?.O, "| Duke Mental:", vDuke?.mental);
      ...
      const payload = {
        ...
        // ── VISTA raw sequences ──────────────────────────────────
        "VISTA Shape": (shapeSeq||[]).join(","),
        "VISTA Color": (colorSeq||[]).join(","),
        "VISTA Shade": (shadeSeq||[]).join(","),
        "VISTA Smiley": (smileySeq||[]).join(","),
        // ── VISTA scored domains ─────────────────────────────────
        "VISTA CQ": r?.d1?.CQ || "",
        "VISTA IQ Band": r?.d1?.iqBand?.band || "",
        "VISTA EQ": r?.d3?.EQSS || "",
        "VISTA EQ Band": r?.d3?.eqBand?.band || "",
        "VISTA Distress": r?.d4?.phqAnalog?.level || "",
        "VISTA Risk": r?.d5?.CRI || "",
        "VISTA BF-O": r?.d2?.BFt?.O || "",
        "VISTA BF-C": r?.d2?.BFt?.C || "",
        "VISTA BF-E": r?.d2?.BFt?.E || "",
        "VISTA BF-A": r?.d2?.BFt?.A || "",
        "VISTA BF-N": r?.d2?.BFt?.N || "",
        // ── VALID scored domains ─────────────────────────────────
        "VALID CQ": vCAT?.iq || "",
        "VALID CQ Band": vCAT?.label || "",
        "VALID Percentile": vCAT?.pctRank || "",
        "VALID BFI-O": vBFI?.O || "",
        "VALID BFI-C": vBFI?.C || "",
        "VALID BFI-E": vBFI?.E || "",
        "VALID BFI-A": vBFI?.A || "",
        "VALID BFI-N": vBFI?.N || "",
        "VALID Physical": vDuke?.phys || "",
        "VALID Mental": vDuke?.mental || "",
        "VALID Social": vDuke?.social || "",
        "VALID General": vDuke?.general || "",
        "VALID Self-Esteem": vDuke?.selfEsteem || "",
        "VALID Anxiety": vDuke?.anxiety || "",
        "VALID Depression": vDuke?.depression || "",
        "VALID Pain": vDuke?.pain || "",
        "VALID Disability": vDuke?.disability || "",
        "VALID CSSRS Level": vCSS?.level ?? "",
        "VALID CSSRS Label": vCSS?.label || "",
        "VALID AUDIT Score": vAUD?.score ?? "",
        "VALID AUDIT Label": vAUD?.label || "",
      };
    ...
    // ══════════════════════════════════════════════════════════════
    // STEP 2: Generate narratives and show report
    // ══════════════════════════════════════════════════════════════
    try {
      if (shapeSeq.length > 0) {
        // If clinical data already computed and in ref, just generate narratives
        if (vistaResultsRef.current?.clinical) {
          const cl = vistaResultsRef.current.clinical;
          const participant = { name:demo?.name||"", age:demo?.age||"", gender:demo?.gender||"",
            edu:demo?.edu||"", language:demo?.language||"", setting:demo?.setting||"" };
          const narr = await vistaGenerateReport(cl, participant);
          const vr = { clinical:cl, narratives:narr, reportId:vistaReportId.current };
          vistaResultsRef.current = vr;
          setVistaResults(vr);
  ...
  if (screen === "report" && (vistaResults || vistaResultsRef.current || validResp)) return (
    <ReportErrorBoundary demographics={demographics} validResp={validResp} vistaResults={vistaResults || vistaResultsRef.current}>
      <CombinedReport vistaSeqs={{ shapeSeq, colorSeq, shadeSeq, smileySeq }}
        vistaResults={vistaResults || vistaResultsRef.current} validResp={validResp} demographics={demographics}/>
    </ReportErrorBoundary>
  );
