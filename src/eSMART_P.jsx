...
export default function App() {
  ...
  const [ci,    setCi]    = useState({ name:"",age:"",gender:"",fileNo:"",regNo:"",school:"",examiner:"",date:new Date().toISOString().slice(0,10),dob:"",informantName:"",relation:"" });
  ...
  function sendSheets() {
    if(!shUrl){setShSt("⚠️ Enter your Google Apps Script URL."); return;}
    setShSt("⏳ Sending...");
    fetch(shUrl,{method:"POST",mode:"no-cors",headers:{"Content-Type":"application/json"},
      body:JSON.stringify({ci,ageGroup:ageInfo.group,p,bx,S,RL:RL.tag,SF,sm,ts:new Date().toISOString()})})
